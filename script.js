...
function buildGallery(artworksData) {
    ...
    if (artworksData.length === 0) {
        galleryGrid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; padding: 4rem 2rem; color: #F5F1E8;"><h3 style="font-family: Playfair Display, serif; font-size: 2rem; margin-bottom: 1rem;">No Artworks Found</h3><p style="color: #D4AF37; font-size: 1.1rem;">Upload your artwork photos to the <strong>images/</strong> folder.<br>Name them: <strong>artwork-1.jpg</strong>, <strong>artwork-2.jpg</strong>, etc.</p></div>';
        return;
    }
    ...
    artworksData.forEach((artwork, index) => {
        ...
        if (artwork.available) {
            ...
            actionsContainer.querySelector('.btn-purchase').addEventListener('click', function(e) { e.stopPropagation(); purchaseArtwork(artwork.id, artwork.title, artwork.price); });
            actionsContainer.querySelector('.btn-inquire').addEventListener('click', function(e) { e.stopPropagation(); inquireArtwork(artwork.id, artwork.title); });
        } else {
